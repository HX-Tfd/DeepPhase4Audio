...
class PAEllaModel(pl.LightningModule):

    def __init__(self, cfg, **kwargs) -> None:
        super(PAEllaModel, self).__init__()
        self.cfg = cfg
        self.g_count = 0
        ...
        # embedding channels must be power of 2!
        if not cfg.embedding_channels == 2** int(math.log2(cfg.embedding_channels)):
            ValueError("Number of embedding channels must be power of 2")
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_total = loss + 0.5*stft_sc + 0.5*stft_mag
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i in range(batch.shape[0]):
            ...
            if  self.cfg.safe_signals and not self.saved_once:
